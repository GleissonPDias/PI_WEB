...
if($novonome != ''){
    try {
        ...
        $stmt = $pdo->prepare('UPDATE sub_categoria SET nome = :novonome where nome = :categoria');
        $stmt->execute([
            ':novonome' => $novonome,
            ':categoria' => $categoria
        ]);

        $stmt = $pdo->prepare('UPDATE produto SET id_sub_categoria = :novonome WHERE id_sub_categoria = :categoria');
        $stmt->execute([
            ':novonome' => $novonome,
            ':categoria' => $categoria
        ]);
